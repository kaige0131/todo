...
app.post("/webhook", async (req, res) => {
  ...
  const eid = body.header && body.header.event_id;
  if (eid) {
  ...
  console.log("事件:", eventType, "消息类型:", msgType, "聊天:", chatType);
  ...
  let content = "";
  try { content = JSON.parse(event.message.content).text || ""; }
  catch(e) { content = event.message.content || ""; }
  const sender = (event.sender && event.sender.sender_id && event.sender.sender_id.open_id) || "未知";
...
app.get("/", (req, res) => res.send("飞书 TO-DO 机器人运行中"));
